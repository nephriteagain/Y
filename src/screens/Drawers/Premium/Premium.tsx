...
const fakeWords = Array.from({length:10}, () => faker.lorem.words({min:1, max: 4}))
        ...
        <View className="p-4 gap-y-4">
            ...
            <ScrollView className="gap-y-2 py-2 px-8">
                ...
                {fakeWords.map((w,i) => (
                    <View key={i} className="flex-row justify-between">
                        <Text className="font-semibold" style={{fontSize: 16}}>{w}</Text>
            ...
            <View className="py-6">
                <Pressable className="bg-black items-center justify-center py-3 rounded-3xl">
                    <Text className="text-white text-xl font-bold">Starting at &#8369;1,250.00</Text>
                </Pressable>
            </View>
        </View>
    )
